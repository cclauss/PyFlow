...
class reroute(NodeBase):
    def __init__(self, name):
        ...
        self.input = self.createInputPin("in", 'AnyPin', structure=StructureType.Single, constraint="1", structConstraint="1")
        self.output = self.createOutputPin("out", 'AnyPin', structure=StructureType.Single, constraint="1", structConstraint="1")
        self.input.checkForErrors = False
        self.output.checkForErrors = False
        self.input.enableOptions(SUPPORTS_EVERYTHING)
        self.output.enableOptions(SUPPORTS_EVERYTHING)
